...
function ListCrowdfunds({project}) {
 ...
 const amountDonated = project.total_donations / ONE_NEAR;
  return <div className='project'>
    <h2>{project.title}</h2> <span className='creator'>{project.creator}</span>
    <h3>description:</h3>
    ...
    <button onClick={()=> {
     ...
     <form onSubmit={donate}>
      <input type="number" value={donationAmount} onChange={(e) => setDonationAmount(e.target.value)}></input>
